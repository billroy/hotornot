...
Add `--news-pump` or set `NEWS_PUMP_ENABLED=1` to fetch names from Google News RSS in the background. When its insertion queue is empty, the pump refills it from the feed, separates Google's related-story HTML into individual headlines, and submits only exact matches from the bundled person-name index through the same judgment path. It uses no additional API or local inference model. Submissions occur at a random exponential interval with a default mean of 15 seconds per item; change that pacing with `--news-pump-mean-seconds`.
