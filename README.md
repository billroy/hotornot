...
Add `--news-pump` or set `NEWS_PUMP_ENABLED=1` to fetch names from Google News RSS in the background. When its insertion queue is empty, the pump refills it from the feed, extracts likely person names from item titles and descriptions, and submits them through the same judgment path at a random exponential interval with a default mean of 15 seconds per item. Change that pacing with `--news-pump-mean-seconds`.
